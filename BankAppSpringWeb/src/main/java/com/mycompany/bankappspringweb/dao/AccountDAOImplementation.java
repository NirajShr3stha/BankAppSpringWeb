...
public class AccountDAOImplementation implements AccountDAO {
    ...
    public boolean addAccount(int accountNumber, String accountName, int accountBalance) {
        if (findAccount(accountNumber) == null) {
            ...
            String sql = "INSERT INTO `dbbank`.`account` (`accountNumber`, `accountName`, `accountBalance`) VALUES ('" + accountNumber + "', '" + accountName + "', '" + accountBalance + "');";
            return db.iud(sql);
        }
        return false;
    ...
    public Accounts findAccount(int accountNumber) {
        String sql = "select * from dbbank.account where accountNumber=" + accountNumber + ";";
    ...
    public boolean depositeAmount(int accountNumber, int depositeBalance) {
        if (findAccount(accountNumber) != null) {
            ...
            String sql = "UPDATE `dbbank`.`account` SET `accountBalance` = '" + acc.getAccountBalance() + "' WHERE (`accountNumber` = '" + accountNumber + "');";
            return db.iud(sql);
        }
        return false;
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    ...
    public int transferAmount(int senderAccountNumber, int recieverAccountNumber, int transferBalance) {
        if ((findAccount(senderAccountNumber) != null) && (findAccount(recieverAccountNumber) != null)) {
            ...
            if (acc1.getAccountBalance() > transferBalance) {
                ...
                String sql1 = "UPDATE `dbbank`.`account` SET `accountBalance` = '" + acc1.getAccountBalance() + "' WHERE (`accountNumber` = '" + senderAccountNumber + "');";

                String sql2 = "UPDATE `dbbank`.`account` SET `accountBalance` = '" + acc2.getAccountBalance() + "' WHERE (`accountNumber` = '" + recieverAccountNumber + "');";
